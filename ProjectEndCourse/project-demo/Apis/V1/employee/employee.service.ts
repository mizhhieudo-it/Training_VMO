...
export class EmployeeService {
    ...
    async CreateAsync(employee: CreateEmployeeDto) {
        const { name, dateOfBirth, address, citizenCode, technology, experience, foreignLanguage, certificate } = employee;
        try {
            let convertTypeIdTech = convertToObject(technology);
            let result = await this._employeeRepo.store(<EmployeeDocument>{ name, dateOfBirth, address, citizenCode, experience, foreignLanguage, certificate, technology: convertTypeIdTech });
    ...
    async GetByIdAsync(id: string) {
        try {
            let result = await this._employeeRepo.getIdEmployeesAsync(new mongoose.Types.ObjectId(id));
            return Promise.resolve(result);
        } catch (error) {
            return Promise.reject(error);
        }
    ...
    async DeleteAsync(id: string) {
        try {
            let result = await this._employeeRepo.remove(new mongoose.Types.ObjectId(id));
            return Promise.resolve(result);
        } catch (error) {
            return Promise.reject(error);
        }
    ...
    async UpdateAsync(id: string, employee: UpdateEmployeeDto) {
        ...
        try {
            let convertTypeIdTech = convertToObject(technology);
            let result = await this._employeeRepo.update(convertId, <EmployeeDocument>{ name, dateOfBirth, address, citizenCode, experience, foreignLanguage, certificate, technology: convertTypeIdTech });
            return Promise.resolve(result);
        } catch (error) {
            return Promise.reject(error);
        }
