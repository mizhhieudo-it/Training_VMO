...
export class GoogleService {
  constructor(private readonly _authService: AuthService,private readonly _userRepo:UserRepository){
  ...
  async signInWithGoogle(req) {
    if (!req.user) {
      ...
      if(userExsit){
        return this._authService.LoginService(<LoginDto>{username:userExsit.email});
      }else{
